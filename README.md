# Hi, I'm Kareem Mousa 👋
### Junior Penetration Tester | eJPT v2 | CCNA v1.1

Passionate Cybersecurity & Systems Engineering graduate specializing in Web Application, Network, and Active Directory Penetration Testing.

---

### 🛡️ Core Technical Skills
- **Network & Infrastructure Security:** Nmap, Wireshark, Metasploit, Active Directory (BloodHound, Mimikatz, Impacket), Cisco Networking
- **Web Application Security:** Burp Suite, SQLmap, OWASP Top 10, PortSwigger Web Security Academy
- **Scripting & Automation:** Python, Bash
- **Operating Systems:** Linux (Red Hat, Kali Linux), Windows Server

---

### 📜 Certifications
- **eJPT v2** (eLearnSecurity Junior Penetration Tester)
- **PEH** (Practical Ethical Hacking)
- **CCNA 200-301** (Cisco Certified Network Associate)
- **Red Hat Linux System Administration 1**

---

### 📂 Highlighted Projects & Write-ups
- 🔗 **[Active Directory Attack & Defense Lab](./AD-Lab-Writeup)** - Full attack path execution (Kerberoasting, Pass-the-Hash, Privilege Escalation).
- 🔗 **[PortSwigger Web Security Academy Solutions](./PortSwigger-Writeups)** - Detailed walk-throughs for 50+ OWASP Top 10 web labs.
- 🔗 **[SmartShield IPS Security Assessment](./SmartShield-IPS)** - Graduation project vulnerability assessment & attack framework.

---

📫 **Connect with me:**  
[LinkedIn](https://linkedin.com/in/your-profile) | [TryHackMe Profile](https://tryhackme.com/p/your-profile) | Email: kareemousa17@gmail.com
